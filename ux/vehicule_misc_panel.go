...
// VehiculeMiscPanel holds the contents of the miscellaneous block on the sheet.
type VehiculeMiscPanel struct {
	unison.Panel
...
// NewVehiculeMiscPanel creates a new miscellaneous panel.
func NewVehiculeMiscPanel(vehicule *model.Vehicule, targetMgr *TargetMgr) *VehiculeMiscPanel {
	m := &VehiculeMiscPanel{
		vehicule:  vehicule,
		targetMgr: targetMgr,
	...
	m.AddChild(NewNonEditablePageField(func(f *NonEditablePageField) {
		if text := m.vehicule.CreatedOn.String(); text != f.Text {
			f.Text = text
	...
	m.AddChild(NewNonEditablePageField(func(f *NonEditablePageField) {
		if text := m.vehicule.ModifiedOn.String(); text != f.Text {
			f.Text = text
			MarkForLayoutWithinDockable(f)
		}
	}))
	/*
		...
		m.AddChild(NewStringPageFieldNoGrab(m.targetMgr, m.prefix+"player", title,
			func() string { return m.vehicule.Profile.PlayerName },
			func(s string) { m.vehicule.Profile.PlayerName = s }))
	*/
	return m
}

// UpdateModified updates the current modification timestamp.
func (m *VehiculeMiscPanel) UpdateModified() {
	m.vehicule.ModifiedOn = jio.Now()
}
