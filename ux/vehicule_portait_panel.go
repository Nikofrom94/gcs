...
// VehiculePortraitPanel holds the contents of the portrait block on the sheet.
type VehiculePortraitPanel struct {
	unison.Panel
	vehicule *model.Vehicule
}

// NewVehiculePortraitPanel creates a new portrait panel.
func NewVehiculePortraitPanel(vehicule *model.Vehicule) *VehiculePortraitPanel {
	p := &VehiculePortraitPanel{vehicule: vehicule}
	p.Self = p
...
func (p *VehiculePortraitPanel) drawSelf(gc *unison.Canvas, _ unison.Rect) {
	r := p.ContentRect(false)
	paint := unison.ContentColor.Paint(gc, r, unison.Fill)
	gc.DrawRect(r, paint)
	if img := p.vehicule.Profile.Portrait(); img != nil {
...
func (p *VehiculePortraitPanel) Sync() {
	// Nothing to do
}

func (p *VehiculePortraitPanel) mouseDown(_ unison.Point, button, clickCount int, _ unison.Modifiers) bool {
...
func (p *VehiculePortraitPanel) fileDrop(files []string) {
	for _, f := range files {
		...
		sheet := unison.Ancestor[*VehiculeSheet](p)
		sheet.undoMgr.Add(&unison.UndoEdit[[]byte]{
			...
			BeforeData: sheet.vehicule.Profile.PortraitData,
			AfterData:  data,
		})
